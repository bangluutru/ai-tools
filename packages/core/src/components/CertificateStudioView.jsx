...
import GoogleFontPicker from './GoogleFontPicker.jsx';
import PromptHelper from './PromptHelper.jsx';
import { CERT_PROMPT_TEXT, CERT_NOTEBOOKLM_PROMPT } from '../utils/prompts.js';
import { uiTranslations } from '../utils/translations.js';
...
export default function CertificateStudioView({ displayLang = 'vi' }) {
  const [data, setData] = useState(() => {
    try {
      const cached = typeof window !== 'undefined' ? localStorage.getItem('docstudio_cert_data') : null;
      if (cached) {
        const parsed = JSON.parse(cached);
        if (Array.isArray(parsed)) return parsed;
      }
    } catch {
      if (typeof window !== 'undefined') localStorage.removeItem('docstudio_cert_data');
    }
