...
export default function SettingsModal({
  ...
  const visibleCount = tools.length - hiddenIds.size;
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 border-b border-slate-800/80 bg-slate-950/40 px-5 py-3 text-xs sm:px-6">
          <span className="font-semibold text-slate-300">Đang hiển thị {visibleCount}/{tools.length} miniapp</span>
        ...
        <div className="custom-scrollbar overflow-y-auto p-3 sm:p-4">
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                      <span>{tool.readiness === 'disabled' ? 'Tạm khóa' : tool.readiness}</span>
                      {tool.defaultVisible === false && <span>• Mặc định ẩn</span>}
                    </div>
                  </div>
